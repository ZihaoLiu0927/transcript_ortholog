...
def getoptions():
    ...
    parser.add_argument("-s", "--separFeature", dest="separFeature", action='store_true', help="output a separate histgram figure for each feature")
    parser.add_argument("-l", "--lengthhist", dest="lengthhist", action='store_true', help="output a distribution histgram figure for length of each feature")
    parser.add_argument("-r", "--ratiohist", dest="ratiohist", action='store_true', help="output a distribution histgram figure for length of each feature")
    parser.add_argument("-v", "--varFeatureNumber", dest="varFeatureNumber", action='store_true', help="output a distribution histgram figure for variable ratio of each feature")
...
def visualize_combined(list_main, list2, list3, df2_ratio, df3_ratio, out, prefix, kbins=10):
    ...
    if max(dat) > 100:
        ...
        lastbin = 101
    bins = [1, 2, 3, 4, 18, 32, 46, 59, 73, 87, 100, lastbin]
    ...
    find_sec_bar = 0
    break_y_up = np.percentile(y_coords, 100/kbins*(kbins-1)) * 0.7
    break_y_down = np.percentile(y_coords, 100/(kbins)*(kbins-2)) * 1.4
    if (break_y_up <= break_y_down):
        for i in range(3, kbins):
            break_y_down = np.percentile(y_coords, 100/(kbins)*(kbins-i)) * 1.4
            if break_y_down < break_y_up:
                find_sec_bar = 1
                break
        if not find_sec_bar:
            print("warning: odd distribution!")
    
    ax1.set_ylim(break_y_up, max(y_coords) * 1.2)
    for i in range(0, len(bins)-1):
...
def main():
    ...
    if not args.filter == None:
        gene_filter = pd.read_csv(args.filter, header = None, squeeze = True)
