...
def getoptions():
    ...
    parser.add_argument("-p", "--prefix", dest="prefix", required=True, help="A preffix for all the output files")
...
def summary_from_files(path, outpath, preffix):
    ...
    df_by_sample.to_csv(outpath + '/' + prefix + "_lima_sampleSum.csv")
        

def main():
    args = getoptions()
    summary_from_files(args.dir, args.out, args.prefix)
    return
